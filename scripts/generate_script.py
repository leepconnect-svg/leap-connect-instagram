...
・method_1〜3では、その用途が一目で伝わる家具/設備を配置する
  ...
  ポップアップストア→ディスプレイ棚、コワーキングスペース→デスク・チェア・PC、
  女子会スペース→ソファ・低めのテーブル・グラス、推し活スペース→ディスプレイ棚・ライト、
  休憩スペース→ソファやベッド・ブランケット、デート利用スペース→落ち着いた照明・ソファ、
  レンタルキッチン→アイランドキッチン・調理器具、Web面接・オンライン商談ブース→
  デスク・PC・背景の壁、シアタールーム→大画面・スクリーン・ソファ、
  楽器練習スタジオ→防音壁・譜面台)

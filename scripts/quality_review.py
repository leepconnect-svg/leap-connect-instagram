...
【タイトル(1枚目に実際に表示される文言)】{title}
【6枚の内容(実際に表示される文言)】
{slides_text}

(参考: 社内の企画段階のテーマ名は「{theme}」でしたが、6枚に収めるために言い回しを調整している場合があります。
実際に画面に表示される上記のタイトル・見出しとの整合性のみで判断してください。企画段階のテーマ名と
最終タイトルの表現が完全一致していないこと自体は減点対象にしないでください)
